...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-2xl">
        <h1 class="text-3xl font-bold mb-6 text-center text-gray-800">Students List</h1>
        <div class="flex justify-end mb-4">
            <a href="{{ route('students.create') }}"
                class="btn bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">Add Student</a>
        </div>
        <ul class="space-y-4">
            @foreach ($students as $student)
                <li class="p-4 bg-gray-50 rounded shadow">
                    <div class="flex justify-between items-center">
                        <div>
                            <p class="text-lg font-semibold text-gray-700">{{ $student->name }}</p>
                            <p class="text-sm text-gray-500">{{ $student->email }}</p>
                            <p class="text-sm text-gray-500">Age: {{ $student->age }}</p>
                            <p class="text-sm text-gray-500">Course: {{ $student->course }}</p>
                        </div>
                        <div class="flex space-x-2">
                            <a href="{{ route('students.edit', $student->id) }}"
                                class="btn bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600">Edit</a>
                            <form action="{{ route('students.destroy', $student->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="btn bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">Delete</button>
                            </form>
                        </div>
                    </div>
                </li>
